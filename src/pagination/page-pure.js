var PageComponent = Vue
		.extend({
			template : ''
					+ '<form class="pure-form pure-menu pure-menu-horizontal">'
					+ 	'<ul class="pure-menu-list" v-show="conf.totalItems > 0">'
					+ 		'<li class="pure-menu-item">'
					+ 			'<button :class="[\'pure-button\', {\'pure-button-disabled\': conf.currentPage == 1}]" @click="prevPage()">&laquo;</button>'
					+ 		'</li>'
					+ 		'<li v-for="item in pageList" track-by="$index" class="pure-menu-item">'
					+ 			'<button :class="[\'pure-button\', {\'pure-button-primary\':item == conf.currentPage, \'pure-button-disabled\':item == \'...\'}]" @click="changeCurrentPage(item)">{{item}}</button>'
					+ 		'</li>'
					+ 		'<li class="pure-menu-item">'
					+ 			'<button :class="[\'pure-button\', {\'pure-button-disabled\': conf.currentPage == conf.numberOfPages}]" @click="nextPage()">&raquo;</button>'
					+ 		'</li>'
					+ 		'<li class="pure-menu-item">&nbsp;</li>'
					+ 		'<li class="pure-menu-item" v-show="conf.totalItems > 0">'
					+ 			'第<input type="text" v-model="jumpPageNum" @keyup.enter="jumpToPage($event)" style="width: 60px;"/>页 '
